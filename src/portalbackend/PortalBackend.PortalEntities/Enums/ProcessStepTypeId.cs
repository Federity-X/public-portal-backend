...
public enum ProcessStepTypeId
{
    ...
    TRIGGER_CALLBACK_OSP_CREATED = 211,
    RETRIGGER_CALLBACK_OSP_CREATED = 212,
    TRIGGER_CALLBACK_OSP_INVITED = 213,
    RETRIGGER_CALLBACK_OSP_INVITED = 214,
    ...
    RETRIGGER_CONNECTOR_SELF_DESCRIPTION_WITH_OUTDATED_LEGAL_PERSON = 808,

    // IdentityHub managed wallet (BE-293). Provisions a holder ParticipantContext + did:web
    // on the IdentityHub, then reuses the existing VALIDATE_DID_DOCUMENT -> TRANSMIT_BPN_DID
    // -> REQUEST_BPN_CREDENTIAL chain (mirrors the DIM/Custodian wallet steps).
    CREATE_IDENTITY_HUB_WALLET = 809,
    RETRIGGER_CREATE_IDENTITY_HUB_WALLET = 810
}
